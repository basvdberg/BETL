...
--------------------------------------------------------------------------------------------------
-- 2021-07-07 BvdB generate object and column definitions for rdw tables and latest views
				  This is based on staging meta data 

clear_meta_data
exec [dbo].[update_rdw_latest_view_def] 
	select * from obj_ext  
	select * from col_ext  where is_definition=1
	order by obj_id, ordinal_position asc 

*/
CREATE procedure [dbo].[update_rdw_latest_view_def] as 

-- BEGIN Generate RDW latest view definitions
...
select left(obj_name, len(obj_name)-4) obj_name, obj_id, server_type_id 
from dbo.obj_ext o
where 
obj_type = 'table'
...
insert into @existing_objects 
	...
	from dbo.Obj
	where parent_id = @parent_id  -- rdw
	and is_definition=1
	and obj_type_id = 20 -- view
...
select @parent_id, o.obj_name, 20 , o.obj_id, 1, server_type_id
from @new_objects o
...
-- END Generate RDW latest view definitions

-- BEGIN Generate RDW latest view column definitions
...
-- END Generate RDW latest view column definitions
