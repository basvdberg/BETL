...
begin
	...
	begin try
		...
		if @debug = 1 exec dbo.log @transfer_id =@transfer_id, @log_type ='var', @msg ='Property scope ?', @i1 =@property_scope, @simple_mode = 1
		if @prop_id is null 
		begin
		...
		begin
			...
			exec dbo.log @transfer_id =@transfer_id, @log_type ='error', @msg ='Property scope ? is not defined in static.Property', @i1 =@prop, @simple_mode = 1
			goto footer
		end
		if isnull(@enabled,0) = 0 
		begin 
			exec dbo.log @transfer_id =@transfer_id, @log_type ='error', @msg ='Property ? is not enabled in static.Property', @i1 =@prop, @simple_mode = 1
		end 
		...
		if @debug = 1 exec dbo.log @transfer_id =@transfer_id, @log_type ='var', @msg ='Lookup ?(?) ', @i1 =@full_obj_name, @i2=@obj_id ,  @simple_mode = 1
		...
		begin
			...
			begin
				...
				select @obj_id = dbo.obj_id(@full_obj_name)
				if @debug = 1 exec dbo.log @transfer_id =@transfer_id, @log_type ='var', @msg ='Created object ?(?) ', @i1 =@full_obj_name, @i2=@obj_id ,  @simple_mode = 1
			end
		else 
			begin
				if @debug = 1 exec dbo.log @transfer_id =@transfer_id, @log_type ='error', @msg ='object not found ?(?) ', @i1 =@full_obj_name, @i2=@obj_id ,  @simple_mode = 1
				goto footer
			end
		end
		...
		if @debug = 1 exec dbo.log @transfer_id =@transfer_id, @log_type ='var', @msg ='property value ?(?) ', @i1 =@prop, @i2=@value ,  @simple_mode = 1
	END TRY
	BEGIN CATCH
	...
	footer:
	if @debug=1 exec dbo.log @transfer_id =@transfer_id, @log_type ='footer', @msg ='DONE ? ? ?->?', @i1 =@proc_name , @i2 =@prop, @i3 =@full_obj_name, @i4=@value, @simple_mode = 1
