...
CREATE procedure [dbo].[parse_handlebars_obj_id] 
	@obj_id as int 
	, @template_name as varchar(255)
	, @batch_id as int =-1 
	, @output as varchar(max) =''	OUTPUT  
	, @output_result as bit =1 -- select @output in the end 

as 

begin 
--declare 	@obj_id as int =184	, @batch_id as int =-1, @template_name as varchar(255) 
	...
	begin 
		if @delete_dt is not null 
			exec log @batch_id, 'ERROR', 'object ? was deleted at ? ', @obj_id , @delete_dt
		else
		begin
			if @obj_id is null 
				exec log @batch_id, 'ERROR', 'obj_id is null', @obj_id 
			else 
				exec log @batch_id, 'ERROR', 'object with id=? does not exist ', @obj_id 
		end
	...
	begin 
		exec log @batch_id, 'ERROR', 'ordinal_position for the columns in object ? is not unique. ', @obj_id 
	...
	if @template is null 
	begin 
		exec log @batch_id, 'ERROR', 'invalid template name ?', @template_name
	...
	BEGIN
		...
		if @close_pos=0 
		begin 
			exec log @batch_id, 'error', 'syntax error no closing mustaches found after positiion ?+2 ', @open_pos 
			goto footer
		end	
...
--		set @suffix = util.suffix(@value,' ')
		...
		exec log @batch_id, 'Progress', '?..?: "?", "?" ?,?', @open_pos, @close_pos, @value ,@c, @node_str, @parent_str
...
--			set @node_str = isnull(@parent.ToString(),'') + '.'+  isnull(@node.ToString() , '') 
			--exec log @batch_id, 'Progress' , 'parent.node ? ', @node_str 
		...
		--exec log @batch_id, 'Progress', '?..?: "?", "?" "?" "?" "?" ', @open_pos, @close_pos, @value ,@helper, @column_name, @condition, @expression
		...
		begin
			...
			set @node_str = @node.ToString() 
			set @parent_str = @parent.ToString()
			exec log @batch_id, 'Progress', '?,?: "?", "?" ?,?', @i, @j, @value ,@next_c, @node_str, @parent_str
			...
			--if @i = 193
			--begin
			--	exec log @batch_id, 'progress', 'AAP c "?", next_c "?" value "?"' , @c, @next_c, @value 
			--	set @node_str = @node.ToString() 
			--	set @parent_str = @parent.ToString()
			...
			if @l>0 
			begin 
				exec log @batch_id, 'Progress', '?+2..?:"?" ? ( type:const)' , @close_pos, @open_pos, @value , @l
				set @node = @parent.GetDescendant(@node, NULL)
				insert into #ast(node,s, type)  values ( @node, @value,  'const') 
			end 
		end
	...
	left join ( 
		select '_batch_id' _name, convert(nvarchar(255), @batch_id)  _value
		union all 
	...
	exec log @batch_id, 'INFO', 'ast merged with object tree to fill expressions'
	...
	exec log @batch_id, 'INFO', 'output generated and conditions evaluated'
