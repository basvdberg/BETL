...
WITH q AS (SELECT        o.obj_id, o.obj_type_id, ot.obj_type, o.obj_name, o.parent_id, parent_o.obj_name AS parent, parent_o.parent_id AS grand_parent_id, grand_parent_o.obj_name AS grand_parent, 
                                                   grand_parent_o.parent_id AS great_grand_parent_id, great_grand_parent_o.obj_name AS great_grand_parent, o._create_dt, o._delete_dt,  o._record_dt, o._record_user,  
    ...
    (SELECT        obj_id, obj_type, obj_name, 
                                ...
                                , _create_dt, _delete_dt, _record_dt, _record_user, parent_id, grand_parent_id, great_grand_parent_id, prefix, obj_name_no_prefix, server_type, server_type_id, identifier,src_obj_id, external_obj_id, _source
      FROM            q AS q_1)
    SELECT        q2_1.obj_id, CASE WHEN obj_type IN ('user', 'server') THEN [obj_name] ELSE ISNULL(quotename(CASE WHEN srv <> 'LOCALHOST' THEN srv ELSE NULL END) + '.', '') + ISNULL(quotename(db), '') 
                              ...
                              q2_1.great_grand_parent_id, q2_1.server_type_id, q2_1._create_dt, q2_1._delete_dt, q2_1._record_dt, q2_1._record_user, q2_1.prefix, q2_1.obj_name_no_prefix, p.default_template_id, q2_1.identifier, src_obj_id, external_obj_id, _source
