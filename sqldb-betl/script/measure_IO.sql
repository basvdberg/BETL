...
select type, write_mbps = sum( write_mbps), 
iops=sum(iops) 
from qpi.file_stats
group by rollup(type)

select 5 * 60 'mb/min' , 5 * 3600 'mb/uur', (5 * 3600)/1000 'Gb/uur'
select 100 / 18
...
select db_name(database_id) db_name, file_name(file_id) file_name,  * from sys.dm_io_virtual_file_stats(null,null)
select * from  sys.dm_db_resource_stats
select * from  sys.dm_exec_requests 
select * from  sys.dm_os_wait_stats 
order by wait_time_ms desc
select * from  sys.dm_user_db_resource_governance 
select  sys.dm_io_virtual_file_stats()
